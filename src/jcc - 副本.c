...
static void chat(struct client_state *csp)
{
   ...
   fwd_count = forward_url(csp, http, fwds);

   for (fwd_idx = 0; fwd_idx < fwd_count; ++fwd_idx) 
   {
	   fwd = fwds[fwd_idx];
	   if (NULL == fwd)
	   {
	   ...
	   */
	   ...
	   /* fwd->forward_host means http proxy (fwd->gateway_host means SOCKS proxy),
	   * http->ssl == 0 means unsafe(https) connection,
	   * so, if we use http proxy (for both http and https connection)
	   * OR unsafe connection (via http proxy, socks proxy or direct to Internet),
	   * run these code.
	   */
...
#ifdef LIBPRIVOXY 
   strcpy_s(path_of_privoxy, 1024, g_privoxy_config_full_path);
#endif
